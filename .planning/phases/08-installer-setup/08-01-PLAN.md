...
7. **Install shell integration:**
   - Copy init scripts from the release archive (version-matched with binaries):
     ```
     ...
     ```
   - The release archive (built by .github/workflows/release.yml) includes `shell-integration/` with all three init scripts
   - This ensures shell integration scripts are always version-matched with the installed binaries
